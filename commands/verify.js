...
module.exports = {
    ...
    execute(message, args) {
        ...
        let sophmoreRole = message.guild.roles.cache.find(roles => roles.name === 'sophmore');
        let juniorRole = message.guild.roles.cache.find(roles => roles.name === 'junior');
        ...
        collector.on('end', collected => {
            ...
            if (checkEmail.endsWith("yourdomainhere.com")) {
                ...
                const collector2 = message.channel.createMessageCollector({ filter, max: 1, time: 150000000 });
                ...
                collector2.on('end', collected => {
                    ...
                    if (userCode == code) {
                        ...
                        const collector3 = message.channel.createMessageCollector({ filter, max: 1, time: 15000 });
                        ...
                        collector3.on('end', collected => {
                            ...
                            if (userGrade == "7") {
                                ...
                                message.channel.send("You have been given access to the server plus the 7th grade role!")
                            } else if (userGrade == "8") {
                                memberTarget.roles.add(eightRole);
                                message.channel.send("You have been given access to the server plus the 8th grade role!")
                            } else if (userGrade == "9") {
                                memberTarget.roles.add(freshRole);
                                message.channel.send("You have been given access to the server plus the freshman role!")
                            } else if (userGrade == "10") {
                                memberTarget.roles.add(sophmoreRole);
                                message.channel.send("You have been given access to the server plus the sophmore role!")
                            } else if (userGrade == "11") {
                                memberTarget.roles.add(juniorRole);
                                message.channel.send("You have been given access to the server plus the junior role!")
                            } else if (userGrade == "12") {
                                memberTarget.roles.add(seniorRole);
                                message.channel.send("You have been given access to the server plus the senior role!")
                            } else {
